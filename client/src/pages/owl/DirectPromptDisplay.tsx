...
export default function DirectPromptDisplay() {
  ...
  // Track the generated prompt
  const [generatedPrompt, setGeneratedPrompt] = useState<GeneratedPrompt | null>(null);
  const [activeTab, setActiveTab] = useState('prompt-generator');
  
  // Handle when the user selects a prompt
  const handleSelectPrompt = (prompt: GeneratedPrompt) => {
    console.log("DirectPromptDisplay: Received prompt:", prompt);
    ...
    toast({
      ...
      description: "Your writing prompt is ready below.",
    });
    
    // Focus on the prompt section
    setTimeout(() => {
      const promptElement = document.getElementById('generated-prompt-section');
      if (promptElement) {
  ...
  const handleNewPrompt = () => {
    ...
    setTimeout(() => {
      const generatorElement = document.getElementById('prompt-generator-section');
      if (generatorElement) {
  ...
  const handleStartWriting = () => {
    if (!generatedPrompt || !location) return;
    
    // Navigate to the writing quest page with the generated prompt
    navigate(`/owl/quest/generated?locationId=${locationId}&promptType=${location.type}&mode=prompt`, {
  ...
  // Now that we've verified location is not null, we can safely use it

  // Find completed quests for this location
            ...
            <TabsContent value="prompt-generator" className="mt-4 space-y-6">
              ...
              {generatedPrompt && (
                <Card 
                  ...
                  className="border-primary/50 border-2 shadow-lg"
                >
                  ...
                  <CardContent className="space-y-4 mt-2">
                    ...
                    {generatedPrompt.suggestedElements && generatedPrompt.suggestedElements.length > 0 && (
                      <div>
                        <h3 className="font-semibold mb-2">Suggested Elements to Include</h3>
                        <ul className="list-disc pl-5 space-y-1">
